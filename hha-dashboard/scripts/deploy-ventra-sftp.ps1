...
if ($outputs.vendor_storage_account_name) {
    ...
    Write-Host "  username: $storageAccount.ventrastdspec"
    Write-Host "  path:     /vendor-inbound/ventra/stdspec/YYYY-MM-DD/"
    Write-Host "  test cmd: sftp $storageAccount.ventrastdspec@$sftpHost"
    Write-Host ""
    Write-Host "Pre-aggregated user (no-PHI path):"
    Write-Host "  username: $storageAccount.ventrapreagg"
    Write-Host "  path:     /vendor-inbound/ventra/preagg/YYYY-MM-DD/"
    Write-Host "  test cmd: sftp $storageAccount.ventrapreagg@$sftpHost"
    Write-Host ""
    Write-Host "NOTE: storage firewall is Deny-by-default with only the"
    Write-Host "deployer IP allowed. Ventra cannot connect until their egress"
    Write-Host "IPs are added: az storage account network-rule add -g <rg>"
    Write-Host "  --account-name $storageAccount --ip-address <ventra-ip>"
} else {
