        ...
        <MetricCard
          ...
          sub={
            <span className="text-red-600 font-semibold">
              {signed(ops.census_variance_vs_avg)} vs 3-mo avg
            </span>
          }
          ...
          <div className="mt-5 border-t border-slate-100 pt-4">
            ...
            <div className="flex items-end gap-1.5">
              {[
                ...
                {
                  label: "31-60",
                  value: ar.fl_buckets.bucket_31_60,
                ...
                {
                  label: "91-120",
                  value: ar.fl_buckets.bucket_91_120,
                  color: "bg-amber-500",
                  h: 28,
                },
            ...
            <div className="mt-2 text-xs text-slate-500">
              ...
              · TX &gt;120d{" "}
              <span className="font-bold text-red-600">{pct(ar.tx_over_120_pct)}</span> · target
              &lt;15%
            </div>
          </div>
        </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader
            ...
            right={<Badge variant="blue">Board 3</Badge>}
          />
          <Row label="H&P within 24h" target={`Target ≥${clin.hp_24h_target}%`}>
            <span
              className={
                clin.hp_24h_pct >= clin.hp_24h_target ? "text-emerald-600" : "text-amber-600"
              }
            >
              {pct(clin.hp_24h_pct)}
            </span>
          ...
          <Row label="DC Summary within 48h" target={`Target ≥${clin.dc_48h_target}%`}>
            <span
              className={
                clin.dc_48h_pct >= clin.dc_48h_target ? "text-emerald-600" : "text-amber-600"
              }
            >
              {pct(clin.dc_48h_pct)}
            </span>
          ...
          <Row label="Credentials expiring <30d" target="Crystal to action">
            <span
              className={clin.credentials_expiring_30d > 0 ? "text-red-600" : "text-emerald-600"}
            >
        ...
        <Card>
          <CardHeader
            ...
            owner="Andrea Simon"
            right={<Badge variant="blue">Board 4</Badge>}
          />
...
function ProgressRow({
  ...
  const textTone =
    pctOf >= 100 ? "text-emerald-700" : pctOf >= 90 ? "text-amber-700" : "text-red-700";
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-sm">
        ...
        <span className={`font-bold tabular-nums ${textTone}`}>
          {compact
            ? `${usd(actual, true)} / ${usd(target, true)}`
            : `${usd(actual)} / ${usd(target)}`}
        </span>
      </div>
...
function MiniStat({
  ...
  value,
  tone,
}: { label: string; value: number | string; tone?: "warn" }) {
  return (
    <div>
      <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{label}</div>
      <div
        className={`mt-0.5 text-2xl font-bold tabular-nums ${tone === "warn" ? "text-amber-600" : ""}`}
      >
        {value}
      </div>
    </div>
